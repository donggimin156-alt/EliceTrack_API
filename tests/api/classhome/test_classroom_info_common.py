...
from api.schemas.classroom_schema import ClassroomSchemas
...
class TestClassroomList:
    ...
    def test_ch_001_list_response(self, request, client_fixture):
        ...
        data = resp.json()
        assert_valid_schema(data, ClassroomSchemas.CLASSROOM_LIST_SCHEMA)
        for item in data:
    ...
    def test_ch_002_missing_all_params(self, request, client_fixture):
        ...
        missing_fields = {item["loc"][-1] for item in detail if item.get("type") == "missing"}
    ...
    def test_ch_007_skip_exceeds_total_returns_empty(self, request, client_fixture):
        ...
        assert isinstance(count_resp.json(), int), count_resp.text
        overflow_skip = count_resp.json() + OVERFLOW_SKIP_BUFFER
    ...
    def test_ch_009_no_auth_returns_403(self, request, client_fixture):
        """[CH-009] Authorization 헤더 없이 목록 API 호출 시 403을 반환해야 한다.
    ...
    def test_ch_010_list_count_consistency(self, request, client_fixture):
        """[CH-010] 목록 API 배열 길이가 count API 값과 일치해야 한다.
        ...
        assert len(list_resp.json()) == total, (
            f"목록 길이({len(list_resp.json())}) ≠ count API 값({total})"
        )
